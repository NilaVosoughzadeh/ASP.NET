...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function validateInput(value) {
        ...
        if (!value) {
            return "Ø§ÛŒÙ† ÙÛŒÙ„Ø¯ Ù†Ù…ÛŒâ€ŒØªÙˆØ§Ù†Ø¯ Ø®Ø§Ù„ÛŒ Ø¨Ø§Ø´Ø¯";
        } else if (!emailPattern.test(value) && !phonePattern.test(value)) {
            return "Ù„Ø·ÙØ§ Ø§ÛŒÙ…ÛŒÙ„ ÛŒØ§ Ø´Ù…Ø§Ø±Ù‡ Ù…ÙˆØ¨Ø§ÛŒÙ„ Ù…Ø¹ØªØ¨Ø± ÙˆØ§Ø±Ø¯ Ú©Ù†ÛŒØ¯";
        }
        return "";
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    confirmPassword?.addEventListener("input", function () {
        if (confirmPassword.value !== passwordInput.value) {
            confirmPassword.setCustomValidity("Ø±Ù…Ø² Ø¹Ø¨ÙˆØ± ØªØ·Ø§Ø¨Ù‚ Ù†Ø¯Ø§Ø±Ø¯");
        } else {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (accordions.length > 0) { // Ø¨Ø±Ø±Ø³ÛŒ ÙˆØ¬ÙˆØ¯ Ø¢Ú©Ø§Ø±Ø¯Ø¦ÙˆÙ† Ø¯Ø± ØµÙØ­Ù‡
